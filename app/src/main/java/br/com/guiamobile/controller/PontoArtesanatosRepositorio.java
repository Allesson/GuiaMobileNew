 ...
 */
public class PontoArtesanatosRepositorio {

    private PontoTuristicoOpenHelper pontoTuristicoOpenHelper;

    public PontoArtesanatosRepositorio(Context context) {
     ...
     */
    public List<PontoTuristico> buscarPontoArtesanatos(String busca) {
        SQLiteDatabase db = pontoTuristicoOpenHelper.getWritableDatabase();

        String selectSQL = "SELECT * FROM tbl_pontoArtesanatos WHERE titulo LIKE '%" + busca + "%'";
    ...
    public List<PontoTuristico> listar(){
        ...
        Cursor cursor=db.query(PontoTuristicoOpenHelper.TBL_PONTO_ARTESANATOS, new String[]{"id", "titulo", "descricao", "texto","latitude","longitude","idTab"},
