 ...
 */
public class PontoPontesRepositorio {

    private PontoTuristicoOpenHelper pontoTuristicoOpenHelper;

    public PontoPontesRepositorio(Context context) {
     ...
     */
    public List<PontoTuristico> buscarPontoPontes(String busca) {
        SQLiteDatabase db = pontoTuristicoOpenHelper.getWritableDatabase();

        String selectSQL = "SELECT * FROM tbl_pontoPontes WHERE titulo LIKE '%" + busca + "%'";
    ...
    public List<PontoTuristico> listar(){
        ...
        Cursor cursor=db.query(PontoTuristicoOpenHelper.TBL_PONTO_PONTES, new String[]{"id", "titulo", "descricao", "texto","latitude","longitude","idTab"},
